...
export async function handleCreateEmployeeMeal(req: AuthRequest, res: Response): Promise<void> {
  const user = req.user!
  const isGoogle = user.platform === 'google'

  if (user.role !== 'LEAD' && user.role !== 'ADMIN') {
    const msg = 'Only leads and admins can create meal records for team members.'
  ...
  const msg = `Meal record created for **${targetName}** on **${data.date}**${wfhNote}`
  res.json(isGoogle ? { text: msg } : { type: 4, data: { content: msg, flags: 64 } })
}
